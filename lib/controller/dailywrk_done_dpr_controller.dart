...
class DailyWrkDone_DPR_Controller extends GetxController {
  ...
  Future dpr_getItemList(int prid,int siteid,int subcontid,String entrytype,BuildContext context) async {
    ...
    await DPRProvider.get_Dpr_ItemList(prid,siteid,subcontid,entrytype).then((value) async {
  ...
  Future SaveButton_EntryScreen_Save(BuildContext context, int id) async {
    ...
    if(checklist == 0) {
      final list = await DPRProvider.SaveIemListScreenEntryAPI(
          formdata, id, aprovedButton,context,imageFiles);
      if (list != null) {
        if (id != 0) {
          if (aprovedButton != 0) {
            clearDatas();
            ...
            Navigator.push(
                ...
                new MaterialPageRoute(
                    builder: (BuildContext context) =>
                    new DailyWork_done_DPR_EntryList()));
            return Navigator.pop(context);
          }
        }
        else {
