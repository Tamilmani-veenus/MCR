...
class SiteVoucher_Controller extends GetxController {
  ...
  Future getSiteVoc_EntryList() async {
    ...
    await Sitevoucher_provider.getSiteVouc_Entry_List(
        loginController.user.value.userId,
        loginController.UserType(),
        SiteVocEntrylistFrDate.text,
        SiteVocEntrylistToDate.text)
        .then((value) async {
      if (value != null && value.length > 0) {
        mainentryList.value = value;
        SiteVocEtyList.value = mainentryList.value;
  ...
  Future getsitevoucherTablesDatas() async {
    Sitevoucher_itemview_GetDbList.clear();
    var siteItem =
    await sitevoucherlistService.SitevoucherItemlist_table_readAll();
  ...
  Future  SaveButtonSitevoucher_ItemlistScreen(BuildContext context, int id) async {
    ...
    final list = await Sitevoucher_provider.SaveSitevoucherScreenEntryAPI(
        id, formdata, imageFiles, buttonControl, context);
    if (list != null) {
      if (id != 0) {
  ...
  Future SitevoucherList_DeleteApi(int VocId, String VocNo) async {
    await Sitevoucher_provider.Sitevoucher_entryList_deleteAPI(
        VocId, VocNo, loginController.UserId(), BaseUtitiles.deviceName)
        .then((value) async {
      if (value != null && value.length > 0) {
        return value;
      }
                  ...
                  Expanded(
                    child: TextButton(
                        onPressed: () {
                          ...
                          SitevoucherList_DeleteApi(SiteVocEtyList[index].vocId,
                              SiteVocEtyList[index].vocNo);
