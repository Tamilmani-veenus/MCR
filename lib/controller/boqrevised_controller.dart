...
class Boq_Revised_Controller extends GetxController {
  ...
  Item_itemlist_textControllersInitiate() {
    ...
    Itemlist_AmountControllers.add(new TextEditingController());
    Addwork_qtyControllers.add(new TextEditingController());
    Addwork_AmountControllers.add(new TextEditingController());
  }
      ...
      if (Addwork_qtyControllers[index].value.text == "0") {
        ...
        Addwork_AmountControllers[index].text =
            (Boqitem_itemview_GetDbList[index].Rate *
                    double.parse(
                        Addwork_qtyControllers[index].value.text.toString()))
                .toStringAsFixed(2);
        updateConsumTables();
      }
    }
  ...
  Future SaveButton_BoqRevisedScreen(BuildContext context, int id) async {
    ...
    final list = await BoqRevised_Provider.SaveBoqRevisedScreenEntryAPI(body, id,context);
    if (list != null) {
      if (id != 0) {
        buttonControl = 0;
        ...
        Navigator.push(
            ...
            new MaterialPageRoute(
                builder: (
                    BuildContext context) => new Boq_Revised_EntryList()));
