...
class DPRProvider{
  ...
  static Future<List<DailywrkDoneDprEntrylistResmodel>> get_dpr_EntryList(int? Userid, String UserType, String frdate, String todate) async {
    var data = null;
    await ApiManager.getAPICall(ApiConstant.GET_DPR_ENTRY_LIST + "?UserId=$Userid&UserType=$UserType&Frdate=$frdate&Todate=$todate")
        .then((value) {
  ...
  static SaveIemListScreenEntryAPI(DailywrkDoneDprItemlistSaveModel data,workId,aprovedButton,context,List<File> imagesPath) async {
    String url;
    var request;
