...
class PunchIn extends StatefulWidget {
  ...
  final String? radius;
  const PunchIn({Key? key, this.latitude, this.longitude, this.radius})
      : super(key: key);
...
//old TM
    ...
    //   setState(() {
    //     loading = false;
    //     punchInController.punchInAddress.value = "${place.name},${place.thoroughfare},${place.subLocality},${place.locality},${place.administrativeArea},${place.postalCode},${place.country}.";
    //     punchInController.punchInLat.value= position.latitude.toString();
    //     punchInController.punchInLon.value= position.longitude.toString();
    //   });
    //   if (kDebugMode) {
    //     print("Punch In Address :: ${punchInController.punchInAddress}");
    //     print("Punch In latitude :: ${punchInController.punchInLat.value}");
    //     print("Punch In longitude :: ${punchInController.punchInLon.value}");
    //   }
    //   if(punchInController.isOnDuty.value==0) {
    ...
    try {
      ...
      final response = await http.get(url, headers: {
        "User-Agent": "NeoHeightsApp/1.0 (https://neoheights.in/contact)"
      });
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          loading = false;
          punchInController.punchInAddress.value = displayName;
          punchInController.punchInLat.value = position.latitude.toString();
          punchInController.punchInLon.value = position.longitude.toString();
          if (kDebugMode) {
  ...
  Future<void> takePicture() async {
    ...
    if (pickedFile != null) {
      ...
      final compressedFile = await FlutterImageCompress.compressAndGetFile(
        ...
        quality: 80, // Try starting between 30–50
        minWidth: 1000, // Adjust resolution (width)
        minHeight: 1000, // Adjust resolution (height)
      );

      if (compressedFile != null) {
        final compressedSizeInBytes = File(compressedFile!.path).lengthSync();
        final compressedSizeInKB = compressedSizeInBytes / 1024;

        print("Compressed Size: ${compressedSizeInKB.toStringAsFixed(2)} KB");

        setState(() {
          punchInController.imageFile.value = XFile(compressedFile.path);
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      onWillPop: () async {
        setState(() {
          if (punchInController.isOnDuty.value == 1 &&
              punchInController.resPunchSts.value == "false") {
            punchInController.isOnDuty.value = 0;
      ...
      child: Scaffold(
        ...
        body: SizedBox(
            ...
            child: apiLoading == "false"
              ...
              children: [
                const Text(
                  "Please capture you photo for punch in",
                  style: TextStyle(
                    fontSize: 12.0,
                    color: Colors.black,
                  ),
                ...
                : Obx(
                  () => Column(
                children: [
                  if (punchInController.imageFile.value !=
                      null)
                    Expanded(
                      flex: 3,
                      child: Padding(
                          ...
                          if (punchInController
                            ...
                            if (punchInController
                              ...
                              BaseUtitiles.showToast(
                                  "Please enter Onduty remarks");
                            } else {
                              if (await BaseUtitiles
                                  .checkNetworkAndShowLoader(
                                  context)) {
