...
class _MRNReportState extends State<MRNReport> {
                    ...
                    child: Row(
                      ...
                      children: <Widget>[
                        ...
                        InkWell(
                          ...
                          onTap: () async {
                            if(_formKey.currentState!.validate()){
                              await download(loginController.user.value.userId!,loginController.user.value.userType!,reportsController.selectedProjectId.value,reportsController.selectedsiteId.value,siteController.FromdateController.text,siteController.TodateController.text,reportsController.projectname.text,reportsController.sitename.text);
                            }
                          },
                        ),
  ...
  download(int uid,String uType,int pId,int SId,String fdate,String todate,String pName,String sName){
     launch( ApiConstant.Web_URL + 'MobileReports/Mobile_Indent_RPT.aspx?UID=$uid&UTYPE=$uType&PID=$pId&SID=$SId&FDT=$fdate&TDT=$todate&PNAME=$pName&SNAME=$sName');
     print(ApiConstant.Web_URL + 'MobileReports/Mobile_Indent_RPT.aspx?UID=$uid&UTYPE=$uType&PID=$pId&SID=$SId&FDT=$fdate&TDT=$todate&PNAME=$pName&SNAME=$sName');
  }

  List<PdfListModel> getMNRAttdList() {
    siteController.getMNRList_Pdf.clear();
    siteController.mrnListValue.forEach((element) async {
      var list = PdfListModel(
        mrnReqNo: element.mrnReqNo,
        mrnReqDate: element.mrnReqDate,
        projectName: element.projectName,
        siteName: element.siteName,
        mrnReqStatus: element.mrnReqStatus,
      );
      siteController.getMNRList_Pdf.add(list);
    });
    return  siteController.getMNRList_Pdf;
  }
  ...
  // }
   ...
   textclear(){
    siteController.siteDropdownName.clear();
    siteController.setSelectedSiteID("--ALL--");
    siteController.selectedsitedropdownName.value="--Select--";
  }
}
