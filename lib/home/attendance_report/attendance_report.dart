...
class _AttendanceReportState extends State<AttendanceReport> {
  ...
  void initState()  {
    ...
    Future.delayed(duration, () async {
      ...
      await reportsController.getSubcontactorReportList(context);
      ...
      child: Scaffold(
        backgroundColor: Setmybackground,
          ...
          children: <Widget>[
                      ...
                      InkWell(
                        ...
                        onTap: () async {
                          if(_formKey.currentState!.validate()){
                            setState(() {
                              attendanceController.attendanceDatas.value.clear();
                              attendanceController.getAttendance_rptList();
                            });
                          }
                        },
                      ),
              ...
              //       ),
              //       onTap: () async {
              //       await download(loginController.user.value.userId!,loginController.user.value.userType!,projectController.selectedProjectId.value,subcontractorController.selectedSubcontId.value,attendanceController.wrktype.value,attendanceController.FromdateController.text,attendanceController.TodateController.text);
              //       },
              //     ),
              //   ],
              // ),
  ...
  download(int uid,String uType,int pId,String pName,int SubId, String wid,fdate,String todate){
    launch(ApiConstant.Web_URL + 'MobileReports/Mobile_SubCont_Attendance_RPT.aspx?UID=$uid&UTYPE=$uType&PID=$pId&PNAME=$pName&SUBID=$SubId&WORKTYPE=$wid&FDT=$fdate&TDT=$todate');
    print(ApiConstant.Web_URL + 'MobileReports/Mobile_SubCont_Attendance_RPT.aspx?UID=$uid&UTYPE=$uType&PID=$pId&PNAME=$pName&SUBID=$SubId&WORKTYPE=$wid&FDT=$fdate&TDT=$todate');
  }
          ...
          Expanded(
            child: Container(
              ...
              child: Obx(()=>ListView.builder(
                  ...
                  itemCount: attendanceController.attendanceDatas.value.length,
                  itemBuilder: (context, index) {
                    return InkWell(
                      onTap: (){
                        attendanceController.OnItemsSelected(attendanceController.attendanceDatas.value[index].attenId,attendanceController.attendanceDatas.value[index].attenNo,context);
                      },
                      child: Container(
                              ...
                              children: <Widget>[
                                    ...
                                    children: <Widget>[
                                      Row(
                                        children: [
                                          ...
                                          Text(
                                            (attendanceController.attendanceDatas.value[index].attenDate.toString()),style: TextStyle(color: Theme.of(context).primaryColor,fontWeight: FontWeight.bold),
                                          ),
                                        ],
                                      ),
                                      Text(attendanceController
                                          .attendanceDatas.value[index].attenNo.toString(),style: TextStyle(color: Theme.of(context).primaryColor,fontWeight: FontWeight.bold),),
                                ...
                                Row(
                                  children:<Widget> [
                                    Expanded(
                                      flex: 2,
                                        child: Text("Project",style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold))),
                                    Expanded(
                                      flex: 3,
                                      child: Text(attendanceController.attendanceDatas.value[index].projectName.toString(),style: TextStyle(color: Colors.black)),
                                    ),
                                ...
                                Row(
                                  children:<Widget> [
                                    Expanded(
                                        flex: 2,
                                        child: Text("SubContractor",style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold))),
                                    Expanded(
                                      flex: 3,
                                      child:Text(attendanceController
                                          .attendanceDatas.value[index].subContName.toString(),style: TextStyle(color: Colors.black),
                                      ),
                                    ),
                                ...
                                Row(
                                  children:<Widget> [
                                    Expanded(
                                        flex: 2,
                                        child: Text("WorkType",style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold))),
                                    Expanded(
                                      flex: 3,
                                      child:Text(attendanceController.attendanceDatas.value[index].WorkType== "NMR" ? "NMR" : "RATE",style: TextStyle(color: Colors.black),
                                      ),
                                    ),
