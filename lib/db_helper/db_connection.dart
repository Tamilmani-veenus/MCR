...
class DatabaseConnection {
  ...
  Future<void> _createDatabase(Database database, int version) async {
    ...
    String subcontDetTable = "CREATE TABLE subcontAttendanceDet (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,catId INTEGER,catName TEXT,wages REAL,nos TEXT,netAmt REAL,remarks TEXT,siteId INTEGER,siteName TEXT,MrgOtHrs REAL,MrgOtAmt REAL,EvgOtHrs REAL,EvgOtAmt REAL,EvgExtrsAmt REAL,Extra REAL)";
    ...
    String transferbetweenItemlistTable = "CREATE TABLE transferbetweenItemlistTable (id INTEGER PRIMARY KEY UNIQUE,materialId INTEGER,materialName TEXT,scale TEXT,stockQty REAL,Qty REAL,balQty REAL,reqDetId INTEGER,rate REAL,amount REAL)";
    await database.execute(transferbetweenItemlistTable);

    String transferbetweenSiteWiseItemlistTable = "CREATE TABLE transferbetweenSiteWiseItemlistTable (id INTEGER PRIMARY KEY UNIQUE,materialId INTEGER,materialName TEXT,scale TEXT,stockQty REAL,Qty REAL,balQty REAL,reqDetId INTEGER,rate REAL,amount REAL)";
    ...
    String materialApprovalListTable = "CREATE TABLE materialApprovalListTable (id INTEGER PRIMARY KEY UNIQUE,materialid INTEGER,reqDetId INTEGER,materialname TEXT,scale TEXT,balqty REAL,reqqty REAL,appqty REAL,apptype TEXT,tranfromprjid TEXT,tranfromprjname TEXT,remarks TEXT,desc TEXT)";
    await database.execute(materialApprovalListTable);

    String materialListTable = "CREATE TABLE materialListTable (id INTEGER PRIMARY KEY UNIQUE,materialid INTEGER,material TEXT,scale TEXT,balqty REAL,qty REAL,stockqty REAL,remarks TEXT,desc TEXT)";
    await database.execute(materialListTable);

    String consumItemListTable = "CREATE TABLE consumItemListTable (id INTEGER PRIMARY KEY UNIQUE,materialid INTEGER,material TEXT,scale TEXT,stockqty REAL,usageqty REAL)";
